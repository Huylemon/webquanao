...
<div style="min-height: 650px">
    ...
    <form role="form" class="text-start" action="{{route('product.update',  $product->id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        @enderror
          <div class="input-group input-group-outline my-3">
            ...
            <input type="text" class="form-control" name="product_color" value="{{$product->color}}">
          </div>
          @error('product_color')
          <div class="text-danger">{{ $message }}</div>
        @enderror
        ...
        <div class="input-group input-group-outline my-3">
          ...
          <input type="number" name="quantities[{{ $q->id }}]"  class="form-control" value="{{$q->pivot->quantity}}" required>
          <input type="hidden" name="sizes[]" value="{{ $q->id }}">
        </div>
        ...
        @endforeach
          <div class="input-group input-group-outline my-3" style="display: inline">
            <p>Mô tả</p>
            <textarea type="text" class="form-control mt-5" name="product_description"style="width: 100%;" id="product_description">{{$product->description}}</textarea>
          </div>
