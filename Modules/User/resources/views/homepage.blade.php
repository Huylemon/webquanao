            ...
            <div class="col">
                <div class="product-grid" data-isotope='{ "itemSelector": ".product-item", "layoutMode": "fitRows" }'>
                    ...
                    <div class="product-item mb-5 {{$p['category_gender'] == 'Nam' ? 'men' : 'women'}}">
                        <a class="product" href="{{url('detailProduct', $p['id'])}}?size=1">
                            <div class="product_image">
                                <img src="{{URL::to('/')}}/images/{{$p['image']}}" alt="" style="max-height:220px">
                            </div>
                            <div class="favorite favorite_left"></div>
                            <div class="product_info">
                                <h6 class="product_name text-truncate" style="margin-top:0"><a href="{{url('detailProduct', $p['id'])}}?size=1">{{$p['name']}}</a></h6>
                                @if(isset($p['discount']) && $p['discount'] > 0)
                                <div class="product_price">{{number_format($p['discount'])}}<span>{{number_format($p['price'])}}</span></div>
                                @else
                                <div class="product_price">{{number_format($p['price'])}}</div>
                                @endif
                            </div>
                        </a>
                ...
                <div class="product_slider_container">
                    <div class="owl-carousel owl-theme product_slider">
                        ...
                        <div class="owl-item product_slider_item">
                            <a class="product-item" href="{{url('detailProduct', $item->id)}}?size=1">
                                <div class="product discount">
                                    <div class="product_image">
                                        <img src="{{URL::to('/')}}/images/{{$item->image}}" alt="" style="max-height: 230px">
                                    </div>
                                    <div class="favorite favorite_left"></div>
                                    @if($item->discount > 0)
                                        <div class="product_bubble product_bubble_right product_bubble_red d-flex flex-column align-items-center"><span>-{{100 - intval(round(($item->discount/$item->price)*100))}}%</span></div>
                                    @endif
                                    <div class="product_info">
                                        <h6 class="product_name text-truncate"><a href="">{{$item->name}}</a></h6>
                                        @if($item->discount > 0)
                                            <div class="product_price">{{number_format($item->discount)}}<span>{{number_format($item->price)}}</span></div>
                                        @else
                                            <div class="product_price">{{number_format($item->price)}}</div>
                                        @endif
                                    </div>
                                </div>
                            </a>
