                ...
                <div class="row">
                    <div class="col-lg-9 image_col order-lg-2 order-1">
                        <div class="single_product_image">
                            <div class="single_product_image_background" style="background-image:url({{URL::to('/')}}/images/{{$products->image}})"></div>
                        </div>
        ...
        <div class="col-lg-4">
            <div class="product_details position-fixed ">
                <div class="product_details_title">
                    <h2 style="font-size: 24px">{{$products->name}}</h2>
                ...
                @if($products->discount > 0)
                    <div class="original_price">{{number_format($products->price)}}</div>
                    <div class="product_price">{{number_format($products->discount)}}</div>
                @else
                    <div class="product_price">{{number_format($products->price)}}</div>
                @endif
                <div class="product_color">
                    <span>Màu sắc: {{$products->color}}</span>
                </div>
                <div class="product_size">
                    ...
                    <div class="d-flex">
                        @foreach($quantities as $q)
                            <form action="{{url('detailProduct', $products->id)}}?size={{$q->id}}" method="post">
                                @csrf
                                <button class="btn btn-light" value="{{$q->id}}">{{$q->size_name}}</button>
                            </form>
                        @endforeach
                    </div>
                </div>
